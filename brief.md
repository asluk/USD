...
Build the first one first. Get it resolving and measured against the authority
below, then get the specification-gap list out of it, and only then start the
second. If you run out of room, that order is the priority order: a working,
measured runtime first, the gap list second, the second architecture last.
...
The proposal is the only specification. **It sits beside this brief as
`proposal.md`. Read that copy and no other.** It is pinned: a branch moves, and
two builds read from a branch cannot be compared. If you fetch the document from
anywhere else, say so — that is a fact about the run.

Read all of it, and `Runtime behavior` inside `Runtime coordinate transformation`
most closely.
...
metre. An implementation that carries absolute coordinates in single precision has
lost more than that spacing before it does any geodesy. Show what your scheme
costs and what the naive one costs, as numbers.
...
4. **Figures, one per claim.** Every claim that can be seen rather than read gets
   ...
   the float32-versus-double comparison, which is a picture and tends to get
   written as a number in a table.

   **Where a claim is about the resolved result, produce the figure from both
   implementations, on the same points.** Two of them drifting apart on real data
   is a failure mode small synthetic scenes do not catch. Where a claim is about
   one architecture — what it caches, when it invalidates — say which one it is
   about.

   Be clear about what that shows. Agreement between the two is evidence that the
   description does not depend on an evaluation architecture. It is not evidence
   of correctness, and it never overrides deliverable 2: both have to match
   closed-form geodesy, and two runtimes agreeing on a wrong answer is the case
   the authority exists to catch.
