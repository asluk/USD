...
_Last updated: 2026-06-23 ~21:40 UTC._
...
5. **NEXT MILESTONE — runtime anchor injection (scene index).** Current model = absolute
   `crs:position` per prim + Cartesian ancestors on top; good for point/grid leaves, but a
   non-georef child of a georef anchor resolves at the origin (anchor's georef lives in
   `crs:position`, which stock USD composition doesn't read). `test_ancestor_compose.py`
   only exercises the inverse (Cartesian ancestor over a georef leaf), confirming the gap.
   Fix = Esri-convergent synthesis: anchor + local-offset, anchor resolved into a
   runtime-INJECTED transform (Hydra scene index), descendants compose as ordinary USD —
   not baked resetXformStack, not per-prim absolutes. Load-bearing for Row 3 (4D twin),
   one-pager C-05 (float32 localization), and the "stock USD sees origin" gap. Outranks the
   validator. (Plan: prototype the injection semantics in Python first to prove the model
   before committing to a C++ Hydra plugin.)
6. **Alongside — projection-engine registration seam** (Simon's Esri-PR ask): PROJ/pyproj as
   one registered engine, not hardcoded; WKT stays opaque to USD.
