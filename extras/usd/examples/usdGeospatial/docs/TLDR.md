...
_Last updated: 2026-06-24 ~00:55 UTC._

## Visual evidence — all 9 figures generated by the codeless build
...
![railway rendered on tiles](railway_render.png)
![per-dataset gallery](datasets_gallery.png)
...
- **generalization.png** — the no-overfit proof: the SAME reference runtime over 7 diverse
...
- **railway_render.png** — an actual RENDER of that real converted asset, drawn entirely from
  runtime-resolved geometry: the 3 geospatial tile ground planes (quadnode-*.png imagery
  sampled onto them) with the Deutsche Bahn rail curves layered on top, in a shared local
  ENU frame. 100% of rail vertices fall within the tile footprint — rails on tiles, via OUR
  resolver. Self-generated in `render_figures.py` (`fig_railway_render`).
- **datasets_gallery.png** — every single-point benchmark (NYC, Sydney, Wellington, Quito,
  Svalbard) gets its own panel: two INDEPENDENT CRS authorings (geographic + projected) resolved
  by the runtime into the SAME ECEF point, shown in local ENU with the E/N axes triad + the
  sub-mm Δ, plus a globe locating all 7 datasets. The table's numbers, made visible.
  Self-generated (`fig_datasets_gallery`).
...
## Changelog
- 2026-06-24 ~01:25 — gave the OTHER 6 datasets visualizations (Aaron asked): `datasets_gallery.png`
  (figure 9) — per-point panels showing the two-CRS (geographic + projected) authorings resolving
  to one ECEF point in local ENU with axes triad + sub-mm Δ, plus a globe locating all 7. Earth-2
  already covered by globe.png/coherence.png.
- 2026-06-24 ~01:20 — added an actual RENDER to the multi-dataset section: `railway_render.png`
  (figure 8), the real converted NVIDIA asset drawn from runtime-resolved geometry — 3 geospatial
  tile ground planes (quadnode imagery) + Deutsche Bahn rail curves on top, shared local ENU,
  100% of rail verts within the tile footprint. Self-generated via `fig_railway_render`.
...
- 2026-06-24 ~00:55 — NO-OVERFIT generalization proof: `generalization_suite.py` runs the
  ...
  REAL NVIDIA Deutsche Bahn railway (Apache-2.0, `data/thirdparty/`) + `convert_omni_geospatial.py`
  (old Omniverse `omni:geospatial:wgs84:*` schema → our crs:binding/crs:position, lat-first
  swap handled). New figure 7 `generalization.png`. README + TLDR updated.
