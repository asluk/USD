...
def author_crs_prim(stage, path, crs: CRS, name: str):
    ...
    # uniform token crs:wkt  (apiName wellKnownText in the Esri schema)
...
def bind_crs(prim, crs_prim):
    ...
    We deliberately do NOT:
      - add the CRS as a reference (composition stays orthogonal to binding)
      - call SetResetXformStack (no runtime behavior baked into scene description)
    """
...
def main():
    ...
    geo_prim  = author_crs_prim(stage, "/World/CRS/WGS84_Geographic3D", CRS_GEO_3D,
                                "WGS84 geographic 3D (EPSG:4979)")
    author_crs_prim(stage, "/World/CRS/WGS84_ECEF", CRS_ECEF,
                    "WGS84 geocentric ECEF (EPSG:4978)")

    # --- Georeferenced sample points: t2m stations placed by (lon, lat, h) ---
    # Each is an Xform BOUND to the geographic CRS, carrying an absolute geodetic
    # position in float64. NO xformOp:translate authored here -- the runtime
    # resolver computes the ECEF cartesian transform.
    pts = UsdGeom.Scope.Define(stage, "/World/GeoSamples")
    n = 0
    sub_lat = lat[::args.stride]
    sub_lon = lon[::args.stride]
    for i, la in enumerate(lat[::args.stride]):
        for j, lo in enumerate(lon[::args.stride]):
            t2m = float(field[i*args.stride, j*args.stride])
            xf = UsdGeom.Xform.Define(stage, f"/World/GeoSamples/p_{i}_{j}")
            p = xf.GetPrim()
            bind_crs(p, geo_prim)
            # absolute geodetic position in the bound CRS (lon, lat, ellipsoidal height)
            pos = p.CreateAttribute("crs:position", Sdf.ValueTypeNames.Double3, custom=False)
            pos.Set(Gf.Vec3d(float(lo), float(la), 0.0))
            # carry the data value as a primvar for color/analysis
