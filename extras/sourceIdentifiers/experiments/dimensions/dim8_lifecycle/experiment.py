...
def render_summary(results, forward_c, roundtrip_c, coexist_c):
    ...
    print('  cost of mechanism plurality.', file=buf)
    print('', file=buf)
    ...
    print('  for the identifier half (intentional collision in the', file=buf)
    print('  proposal). Neutral layer-text inspection cannot distinguish', file=buf)
    print("  A's authoring from D's identifier-half authoring on this", file=buf)
    print('  surface alone.', file=buf)
    print('', file=buf)
